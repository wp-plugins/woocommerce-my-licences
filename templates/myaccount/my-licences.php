...
if ( isset( $_GET['deactivate'] ) ) {
	...
	if ( is_wp_error( $response ) ) {
		$message = apply_filters( 'wc_my_licence_deactivation_error_message', __( 'Unable to deactivate at this time. If this error persist, please contact us. Thank you!', 'ss-wc-my-licences' ) );
		$type    = 'error';
	} else {
		$message = apply_filters( 'wc_my_licence_deactivation_success_message', __( 'You have deactivated', 'ss-wc-my-licences' ) . ' ' . get_the_title( $_GET['deactivate'] ) . ' ' . __( 'successfully for', 'ss-wc-my-licences' ) . ' <strong>' . $_GET['instance'] . '</strong>' );
		$type    = 'success';
	}
...
<table class="shop_table my_account_licences">
	<thead>
		<tr>
			<th class="product" colspan="2"><span class="nobr" style="text-align:center;"><?php _e( 'Product', 'ss-wc-my-licences' ); ?></span></th>
			<th class="licence-key" colspan="2"><span class="nobr" style="text-align:center;"><?php _e( 'Key', 'ss-wc-my-licences' ); ?></span></th>
			<th class="software-version" colspan="2"><span class="nobr" style="text-align:center;"><?php _e( 'Software Version', 'ss-wc-my-licences' ); ?></span></th>
			<?php if ( $variables == 'yes' ) { ?><th class="licence-type" colspan="2"><span class="nobr" style="text-align:center;"><?php _e( 'Type', 'ss-wc-my-licences' ); ?></span></th><?php } ?>
			<th class="activations-remaining" colspan="2"><span class="nobr" style="text-align:center;"><?php _e( 'Activations Remaining', 'ss-wc-my-licences' ); ?></span></th>
		</tr>
	</thead>
	...
	foreach ( $licence_keys as $licence_key ) {
		...
		<tr class="licences">
			...
			<?php if ( $variables == 'yes' ) { ?><td class="licence-type" colspan="2">
			<?php
			if ( isset( $item['variation_id'] ) && $item['variation_id'] > 0 ) {
				...
				if ( ! empty( $licence_type ) ) {
					if ( $licence_type == 'single-licence' ) { $type = __( 'Single site licence', 'ss-wc-my-licences' ); }
					else if ( $licence_type == 'up-to-five-sites' ) { $type = __( 'Up to five sites', 'ss-wc-my-licences' ); }
					else if ( $licence_type == 'unlimited-sites' ) { $type = __( 'Unlimited sites', 'ss-wc-my-licences' ); }
				} else {
					$type = '';
		...
		<tr class="activation <?php echo ( $activation->activation_active ) ?  'active' : 'not-active'; ?>">
			...
			<td colspan="<?php if ( $variables == 'yes' ) { echo '4'; } else { echo '2'; } ?>"><?php if ( $activation->activation_active == 1 ) { ?><a href="<?php echo get_permalink( $post->ID ); ?>?deactivate=<?php echo $activation->software_product_id; ?>&amp;email=<?php echo urlencode( $activation->activation_email ); ?>&amp;instance=<?php echo urlencode( $activation->instance ); ?>&amp;licence_key=<?php echo $activation->licence_key; ?>" class="button deactivate"><?php _e( 'Deactivate', 'ss-wc-my-licences' ); ?></a><?php } ?></td>
		</tr>
		<?php
...
<?php } else { ?>
	<p><?php _e( 'You have no licences.', 'ss-wc-my-licences'); ?></p>
<?php } ?>
